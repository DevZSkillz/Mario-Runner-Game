...
function renderMario() {
  ctx.drawImage(
    mario,
    marioObj.x,
    marioObj.y - marioObj.jumpHeight,
    marioObj.width,
    marioObj.height
  );
}
...
function jump() {
  if (marioObj.jumpHeight < 80) {
    ...
    marioObj.y -= marioObj.jumpSpeed; // Update Y coordinate
...
function renderPlatforms() {
  for (let i = 0; i < platformObj.array.length; i++) {
    ctx.drawImage(
      platform,
      platformObj.array[i].x,
      platformObj.array[i].y,
      platformObj.width,
      platformObj.height
    );
  }
...
function checkHpCollision(player, object, hp) {
  if (
    player.x + player.width > object.x + 25 &&
...
function checkBulletsCollision() {
  for (let i in bulletsArr) {
    for (let k in enemyArr) {
      if (
        bulletsArr[i].x + bulletsArr[i].width > enemyArr[k].x &&
